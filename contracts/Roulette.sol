...
abstract contract Roulette {
     ...
     */
    bool public endedAlreadyExecuted;
     ...
     */
    constructor(uint c, uint token_, uint tax_, uint duration) {
    ...
    function makeBet(uint color) external payable {
        ...
        require(block.number <= validUntil, "Fora do periodo de apostas");
        // Verifica que o valor transferido compra pelo menos uma ficha.
        require(msg.value >= token + tax, "Transferencia insuficiente");
        // Verifica que este endereço não tem um aposta feita em uma outra cor.
        require((bets[msg.sender].registered && bets[msg.sender].color == color) || !bets[msg.sender].registered, "Aposta ja realizada em outra cor");
    ...
    function endContract() external {
        require(!endedAlreadyExecuted, "O fim do contrato foi executado.");
        require(hasEnded(), "O contrato ainda nao terminou.");
    ...
    function donate() external payable {
        require(!hasEnded(), "Contrato ja terminou.");
        totalPrize += msg.value;
        totalDonation += msg.value;
    }

    function bettedOnColor(uint colorId) external view returns(uint){
        require(colorId < nColors, "Cor invalida.");
